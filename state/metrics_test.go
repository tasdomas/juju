...
func (s *MetricSuite) TestCleanupMetrics(c *gc.C) {
	...
	err = s.State.CleanupOldMetrics(s.State.ModelTag())
	c.Assert(err, jc.ErrorIsNil)
...
func (s *MetricSuite) TestCountMetrics(c *gc.C) {
	...
	unsent, err := s.State.CountOfUnsentMetrics(s.State.ModelTag())
	c.Assert(err, jc.ErrorIsNil)
...
func (s *MetricSuite) TestMetricsToSend(c *gc.C) {
	...
	s.Factory.MakeMetric(c, &factory.MetricParams{Unit: s.unit, Sent: false, Time: &now, Metrics: m})
	s.Factory.MakeMetric(c, &factory.MetricParams{Unit: s.unit, Sent: true, Time: &now, Metrics: m})
	result, err := s.State.MetricsToSend(s.State.ModelTag(), 5)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *MetricSuite) TestMetricsToSendBatches(c *gc.C) {
	...
	for i := 0; i < 3; i++ {
		result, err := s.State.MetricsToSend(s.State.ModelTag(), 2)
		c.Assert(err, jc.ErrorIsNil)
		c.Assert(result, gc.HasLen, 2)
...
func (s *CrossModelMetricSuite) TestMetricsAcrossEnvironments(c *gc.C) {
	...
	unsent, err := s.State.CountOfUnsentMetrics(s.models[0].state.ModelTag())
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(unsent, gc.Equals, 1)

	toSend, err := s.State.MetricsToSend(s.models[0].state.ModelTag(), 10)
	c.Assert(err, jc.ErrorIsNil)
	...
	sent, err := s.State.CountOfSentMetrics(s.models[0].state.ModelTag())
	c.Assert(err, jc.ErrorIsNil)
	c.Assert(sent, gc.Equals, 1)

	err = s.State.CleanupOldMetrics(s.models[0].state.ModelTag())
	c.Assert(err, jc.ErrorIsNil)
